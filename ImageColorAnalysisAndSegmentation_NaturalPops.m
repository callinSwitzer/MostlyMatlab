%% Image segmentation for Federico, mapping populations
% Callin Switzer
% 10 June 2016
...
cd(PathName);
flList = dir('*.jpg');
flList(strncmp({flList.name}, '.', 1)) = []; % remove hidden files
fls = {flList.name};
length(fls)
...
for kk = 1:length(fls)
    ...
    try
        ...
        % add black border
        BWsdil(1:size(BWsdil, 1), 1:2) = 0;
        BWsdil(1:size(BWsdil, 1), size(BWsdil, 2) - 1:(size(BWsdil, 2))) = 0;
        BWsdil(1:4, 1:size(BWsdil, 2)) = 0;
        BWsdil(size(BWsdil, 1) - 3:size(BWsdil, 1), 1:size(BWsdil, 2)) = 0;
        %imshow(BWsdil)
...
%         %imshow(BWnobord), title('cleared border image');
        ...
        % erode images
        seD = strel('disk',6);
        BWfinal = imerode(new,seD);
        BWfinal = imerode(BWfinal,seD);
        BWfinal = imerode(BWfinal,seD);
       
       
        
%         BWFill2 = imerode(BWFill1, seD); % remove "tails"
%         BWfinal = imdilate(BWFill2, seD); 
%         
